...
ifeq ($(ARCH),amd64)
	...
	$(DOCKER) build --no-cache --label git_remote=$(shell git remote get-url origin) --label git_commit_id=$(shell git rev-parse HEAD) -t $(MULTI_ARCH_IMG):$(TAG) $(TEMP_DIR)/rootfs
